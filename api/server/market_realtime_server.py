...
Author: chenlei  
Date: 2025-08-14
Description: 实时市场数据股票仪表盘服务 - 基于新框架重构版本
功能: 提供实时市场数据、股票数据展示、实时涨幅分析、涨停监控等功能
"""
...
class MarketRealtimeStockServer(BaseStockServer, SourceDataLogicMixin):
    """实时市场数据股票服务器 - 继承自BaseStockServer，使用基类的缓存机制，集成配置驱动架构"""
    
    def __init__(self, server_type="market_realtime", port=None, auto_update_config=None):
        # 动态获取服务器配置
        server_config = get_server_config(server_type)
        
        # 使用配置文件中的端口，如果未指定则使用参数或默认值
        if port is None:
            port = server_config.get("port", 5009)
        ...
        server_name = server_config.get("name", "实时市场数据")
    ...
    def register_custom_routes(self):
        ...
        if hasattr(self, 'component_manager') and self.component_manager:
            ...
            # 用于跟踪已注册的API路径，避免重复注册
            registered_api_paths = set()
            ...
            for component_id, component_config in components.items():
                ...
                # 如果这个API路径已经注册过，跳过重复注册
                if api_path in registered_api_paths:
                    self.logger.info(f"跳过重复的API路径: {api_path} (组件: {component_id})")
                    continue
                ...
                # 记录已注册的API路径
                registered_api_paths.add(api_path)
                
                self.logger.info(f"已注册路由: {api_path} -> {method_name}")
                registered_count += 1
            
            self.logger.info(f"配置驱动路由注册完成，共注册 {registered_count} 个路由，跳过 {len(components) - registered_count} 个重复或禁用的组件")
...
def main():
    """主函数"""
    print("🚀 启动实时市场数据股票仪表盘服务器...")
    
    # 简单配置：禁用自动更新
    auto_update_config = {'enabled': False}
    
    # 创建服务器实例 - 使用动态配置
    server = MarketRealtimeStockServer(
        server_type="market_realtime",  # 动态服务器类型
