...
class MarketReviewStockServer(BaseStockServer, SourceDataLogicMixin):
    """复盘页面股票服务器 - 继承自BaseStockServer，使用基类的缓存机制，集成配置驱动架构"""
    
    def __init__(self, server_type="market_review", port=None, auto_update_config=None):
        # 动态获取服务器配置
        server_config = get_server_config(server_type)
        
        # 使用配置文件中的端口，如果未指定则使用参数或默认值
        if port is None:
            port = server_config.get("port", 5008)
        ...
        server_name = server_config.get("name", "复盘页面")
...
def main():
    """主函数"""
    print("🚀 启动复盘页面股票仪表盘服务器...")
    
    # 简单配置：禁用自动更新
    auto_update_config = {'enabled': False}
    
    # 创建服务器实例 - 使用动态配置
    server = MarketReviewStockServer(
        server_type="market_review",  # 动态服务器类型
