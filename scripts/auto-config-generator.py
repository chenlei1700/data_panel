...
"""
    ...
    def generate_vscode_tasks(self):
        ...
        for service in self.config['services']:
            if service['enabled']:
                backend_tasks.append(f"{service['taskLabel']} (端口{service['port']})")
        ...
        for service in self.config['services']:
            if service['enabled']:
                task = {
                    "label": f"{service['taskLabel']} (端口{service['port']})",
                    "type": "shell",
                    "command": f"& {self.config['developmentConfig']['pythonPath']} {api_base_path}/{service['serverFile']} {service['port']}",
                    "isBackground": True,
                    "problemMatcher": [],
                    "presentation": {
                        "reveal": "always",
        ...
        utility_tasks = [
            ...
            },
            {
                "label": "重新生成配置",
                "type": "shell",
                "command": "python scripts/auto-config-generator.py",
                "isBackground": False,
                "problemMatcher": [],
                "presentation": {
                    "reveal": "always",
            ...
            },
            {
                "label": "添加新页面",
                "type": "shell",
                "command": "python scripts/quick-add-page.py",
                "isBackground": False,
                "problemMatcher": [],
                "presentation": {
                    "reveal": "always",
...
"""
        ...
        for service in self.config['services']:
            if service['enabled']:
                # 使用相对路径，并传递端口参数
                bat_content += f"""start "{service['name']} (端口{service['port']})" cmd /k "echo 启动{service['name']}服务... && {self.config['projectInfo']['pythonExecutable']} api/{service['serverFile']} {service['port']}"
timeout /t 2 >nul
...
gnome-terminal --title="{service['name']} (端口{service['port']})" -- bash -c "echo '启动{service['name']}服务...'; python3 api/{service['serverFile']} {service['port']}; exec bash" &
sleep 2
...
def add_new_service():
    ...
    print()
    print("🚀 运行以下命令生成所有配置:")
    print("   python scripts/auto-config-generator.py")
...
用法:
  python scripts/auto-config-generator.py          # 生成所有配置文件
  python scripts/auto-config-generator.py add      # 交互式添加新服务
  python scripts/auto-config-generator.py --help   # 显示帮助信息
